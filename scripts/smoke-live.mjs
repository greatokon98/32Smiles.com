#!/usr/bin/env node
// Live smoke suite — runs against a local dev server (localhost:3001) backed
// by a SCRATCH Postgres database (smiles_live_test). No production data is
// touched. Covers: public routes, shop E2E, auth/RBAC (5 roles), admin sections,
...
const DEV_LOG = "/tmp/smiles-live-dev.log"
...
function psql(sql) {
  return execSync(
    `/Applications/Postgres.app/Contents/Versions/latest/bin/psql -d smiles_live_test -tAc "${sql.replace(/"/g, '\\"')}"`,
    { encoding: "utf8" }
  ).trim()
}

// Abort unless we are provably on the scratch DB — never run this against prod.
const scratchCheck = psql("SELECT current_database()")
if (scratchCheck !== "smiles_live_test") {
  console.error(`ABORT: smoke must run against scratch DB 'smiles_live_test', got '${scratchCheck}'`)
  process.exit(2)
}
...
// currency sanity on products page
{
  ...
  const hasNGN = /₦/.test(html) && !/₹/.test(html)
  report("Public", "products page shows ₦ (NGN)", hasNGN, hasNGN ? "" : "missing ₦ / found ₹")
}

// gallery: DB photos + fallback images render, no blank tiles
{
  await goto("/gallery")
  const imgCount = await page.locator("img[src]").count()
  const blankSrc = await page.locator('img[src=""]').count()
  report("Public", "gallery renders ≥9 image tiles (4 DB + 9 fallback)", imgCount >= 9, `imgs=${imgCount}`)
  report("Public", "gallery has no blank-image tiles", blankSrc === 0, `blank=${blankSrc}`)
}
...
console.log("\n== Login redirect (callbackUrl) ==")
{
  ...
  await Promise.all([
    p2.waitForURL((u) => u.pathname === "/checkout", { timeout: 30000 }).catch(() => {}),
  ...
  await p2.fill("#password", accounts.patient[1])
  await Promise.all([
    p2.waitForURL((u) => u.pathname === "/dashboard", { timeout: 30000 }).catch(() => {}),
    p2.click('button[type="submit"]'),
  ])
  await p2.waitForTimeout(400)
  ...
  await p2.fill("#password", accounts.patient[1])
  await Promise.all([
    p2.waitForURL((u) => u.pathname === "/dashboard/orders", { timeout: 30000 }).catch(() => {}),
    p2.click('button[type="submit"]'),
  ])
...

{
  ...
  await page.fill('input[type="tel"]', "+2348111222333")
  await page.locator("textarea").first().fill("12, Adeola Odeku Street, Victoria Island, Lagos")
  await page.getByRole("button", { name: /^Place Order/ }).click()
  await page.waitForURL(/\/order\/confirmation\//, { timeout: 20000 }).catch(() => {})
  const placed = await page.getByText("Order Placed Successfully!").waitFor({ state: "visible", timeout: 30000 }).then(() => true).catch(() => false)
...
let patientOrderNumber = ""
{
  ...
  await page.getByRole("button", { name: "Proceed to Checkout" }).click()
  await page.waitForURL(/\/checkout/)
  await page.locator("textarea").first().fill("14, Admiralty Way, Lekki Phase 1, Lagos")
  await page.getByRole("button", { name: /^Place Order/ }).click()
  await page.waitForURL(/\/order\/confirmation\//, { timeout: 20000 }).catch(() => {})
  const placed = await page.getByText("Order Placed Successfully!").waitFor({ state: "visible", timeout: 30000 }).then(() => true).catch(() => false)
  ...
  if (placed) {
    ...
    report("Shop", "patient orders show ₦ (NGN)", inr)
  }
}
...
let inlineOrderNumber = ""
{
  ...
  if (placed) {
    ...
    const html = await page.content()
    report("Shop", "superadmin confirmation shows ₦ (NGN)", /₦/.test(html) && !/₹/.test(html))
  }
}

{
  ...
  await page.locator('input[type="tel"]').fill("+2348011223344")
  await page.locator("textarea").first().fill("12, Adeola Odeku Street, Victoria Island, Lagos")
  await page.getByRole("button", { name: /^Place Order/ }).click()
  await page.waitForURL(/\/order\/confirmation\//, { timeout: 20000 }).catch(() => {})
  ...
  report("Shop", "filled phone persisted to profile", dbPhone === "+2348011223344", `db="${dbPhone}"`)
}
...
console.log("\n== Stale-cart handling ==")
{
  ...
  await page.evaluate(() => {
    localStorage.setItem(
      "32smiles_cart",
      JSON.stringify([
        { productId: "bogus-product-id-000", title: "Ghost Item", price: 100, quantity: 1, imageUrl: "", currency: "NGN" },
      ])
    )
  })
  await goto("/checkout")
...
let apptId = ""
{
  ...
  const booking = await page.evaluate(
    ...
    { email: accounts.patient[0], phone: "+2348011223344", date: tomorrow }
  )
...
if (patientOrderNumber) {
  ...
  // warm up the mark-read route so a dev cold-compile can't race the assertion
  await page.evaluate(() =>
    fetch("/api/user/notifications/bogus-warmup-id", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
console.log("\n== No-leaks / cleanup ==")
{
  ...
  const hardSeen = [...new Set(globalErrors.filter((e) => !e.startsWith("console:") || !/401|Unauthorized|404|409|ERR_NETWORK_IO_SUSPENDED/.test(e)))]
  const hard = hardSeen.filter((e) => !/401|Unauthorized|404|409|aborted|net::ERR_ABORTED|ERR_NETWORK_IO_SUSPENDED/.test(e))
