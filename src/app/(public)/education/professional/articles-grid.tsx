...
export function ProfessionalArticlesGrid({ articles }: Props) {
  if (articles.length === 0) {
    return (
      <div className="text-center py-16">
        <GraduationCap className="h-12 w-12 text-gray-300 mx-auto mb-4" />
        <p className="text-gray-500 text-lg">
          Professional education articles coming soon. Check back later!
        </p>
      </div>
    )
  }
      ...
      {articles.map((article) => {
        ...
        return (
          <motion.article key={article.id} variants={item}>
            <Link
              href={`/education/professional/${article.slug}`}
            ...
            >
              <div className="relative aspect-[16/10] bg-gradient-to-br from-primary-50 to-primary-100 overflow-hidden">
                ...
                {ea?.educationType && (
                  <span className="absolute top-3 right-3 bg-white/90 text-primary-700 text-xs font-medium px-3 py-1 rounded-full backdrop-blur-sm">
                    {ea.educationType}
                  </span>
                )}
              </div>
              ...
              <div className="p-6">
                ...
                <span className="mt-4 text-primary-600 text-sm font-semibold inline-flex items-center gap-1 group-hover:gap-2 transition-all">
                  Read Article
                </span>
              </div>
            </Link>
