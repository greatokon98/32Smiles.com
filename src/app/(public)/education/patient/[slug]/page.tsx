...
import { ChevronRight, Clock, BookOpen, ArrowRight, HelpCircle } from "lucide-react"
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = seo?.metaTitle || `${article.title} | ${siteConfig.name} Patient Education`
  const description =
    seo?.metaDescription ||
    article.excerpt ||
    `Learn about ${article.title} from ${siteConfig.name}.`
...
export default async function PatientArticlePage({ params }: Props) {
  const { slug } = await params
  const article = await getArticle(slug)
    ...
    <>
      ...
      <section className="relative bg-gradient-to-br from-teal-600 to-teal-800 text-white overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 py-16 lg:py-24">
          <nav className="flex items-center gap-1 text-sm text-teal-200 mb-8">
            <Link href="/" className="hover:text-white transition-colors">
              Home
            </Link>
            <ChevronRight className="h-4 w-4" />
            <Link href="/education/patient" className="hover:text-white transition-colors">
              Patient Education
            </Link>
            <ChevronRight className="h-4 w-4" />
          ...
          <div className="max-w-3xl">
            {ea?.educationType && (
              <span className="inline-block bg-white/10 text-teal-100 text-sm font-medium px-3 py-1 rounded-full mb-4 backdrop-blur-sm">
                {ea.educationType}
              </span>
            )}
            ...
            <div className="flex items-center flex-wrap gap-4 text-sm text-teal-100">
              {author && (
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <article className="lg:col-span-2">
              {article.excerpt && (
                <p className="text-xl text-gray-600 leading-relaxed mb-8 font-medium border-l-4 border-teal-500 pl-6">
                  {article.excerpt}
                </p>
              )}
              ...
              {article.body && (
                <div
                  className="text-gray-700 leading-relaxed
                    ...
                    [&_blockquote]:border-l-4 [&_blockquote]:border-teal-300 [&_blockquote]:pl-6 [&_blockquote]:italic [&_blockquote]:text-gray-500 [&_blockquote]:my-6
            ...
            <aside className="space-y-6">
              <div className="bg-white border border-gray-200 rounded-2xl p-6 sticky top-24">
                ...
                <div className="space-y-3">
                  ...
                  <Link
                    href="/education/patient"
                    className="w-full border-2 border-teal-600 text-teal-600 py-3 rounded-lg font-semibold hover:bg-teal-50 transition-colors flex items-center justify-center gap-2"
                  >
                    More Articles
                  </Link>
                </div>
          ...
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">
                Continue learning with more patient education resources.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedArticles.map((related) => {
                ...
                return (
                  <Link
                    key={related.id}
                    href={`/education/patient/${related.slug}`}
